...
3. **Change code** — Codex natively enforces read-before-edit. The `PostToolUse` hook runs `python -m py_compile` on every written `.py` file and surfaces syntax errors immediately.
...
| **V6 — Docs** | If the change introduced any invariant, gotcha, or decision: update `AGENTS.md`, per-item doc, or memory file in the same commit. | Written down, not just in my head. |
...
## Periodic self-review (AGENTS.md + memory upkeep)

**Rule:** at natural checkpoints Codex must pause, scan the recent conversation in its current context, and proactively suggest additions or corrections to `AGENTS.md` and the memory files. Never silently auto-apply — always propose the exact diff and wait for Pascal's approval.
...
Codex-session-driven golden-file UI + calculation regression. Complements the `simulator.test_*` suites, doesn't replace them.

```
regression/
  README.md                      how it works
  playbook.md                    step-by-step recipe Codex follows
...
Configured in `.Codex/settings.json`; scripts in `.Codex/hooks/`. Hook config is loaded at session start, so changes here require `/hooks` or a new session to take effect in the current session.
...
A `testsim` account is created in the seeded DB for Playwright-driven tests. Credentials and a one-line recreate command live at `.Codex/test-credentials.json` (gitignored). If the db volume is reset, recreate with the snippet inside that file.
